...
def weather_request(areacode):
    ...
    payload = {"areacode": areacode, "inquiry": "duration", "start": "20221101", "end": "20221114"}
...
def get_city_data():
    ...
    f = open('dim_area_V2.csv', 'r', encoding='gbk')
    ...
    for i in area_csv[1:]:
        # 判断是否为市级地区
        # 10.21 根据口径新增123个地市/区县级 exclude列筛选2
        if int(i.split(",")[12]) > 0:
...
def get_city_weather_data(city_list):
    ...
    for i in range(len(city_list)):  # 遍历市级数据列表,爬取天气数据
        ...
        try:
            ...
            for result in request_data['result']['hisWeather']:  # 遍历获取天气信息加入wea_ls
...
def save_to_csv(weather_data):
    ...
    df.to_csv('weather3.csv', index=False, header=False)  # 写入CSV
...
logging.basicConfig(level=logging.INFO,
                    ...
                    filename=f'{str(datetime.datetime.today())[:10]}_weather_log.txt',
                    filemode='w')
